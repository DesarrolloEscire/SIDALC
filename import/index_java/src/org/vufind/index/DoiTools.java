...
public class DoiTools
{
    ...
    public Set<String> getDataText(final Record record, String fieldSpec) {
        // Initialize our return value:
        Set<String> result = new LinkedHashSet<String>();
        ...
        for (String current: input) {
            // If the base URL is found in the string, crop it off for our DOI!
                current.toString().toUpperCase();
                // result = current;
            // if (current.startsWith(baseUrl)) {
                result.add(current.toString().toUpperCase());
            // }
        }
        String sa = new String("minúsculas convertidas en mayúsculas");
        sa.toUpperCase();
        // If we found no matches, return null; otherwise, return our results:
        return result.isEmpty() ? null : result;
    }
