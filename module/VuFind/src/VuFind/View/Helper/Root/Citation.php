    ...
    implements TranslatorAwareInterface
{
    ...
    public function getCitationSIDALC()
    {
        $sidalc = [
            'title' => $this->getAPATitle(),
            'authors' => $this->getAPAAuthors(),
        ...
        // Show a period after the title if it does not already have punctuation
        // and is not followed by an edition statement:
        $sidalc['periodAfterTitle']
            = (!$this->isPunctuated($sidalc['title']) && empty($sidalc['edition']));
        if ($doi = $this->driver->tryMethod('getCleanDOI')) {
            $sidalc['doi'] = $doi;
        }
        ...
        // Behave differently for books vs. journals:
        if (empty($this->details['journal'])) {
            $sidalc['publisher'] = $this->getPublisher(false);
            $sidalc['year'] = $this->getYear();
            return $partial('Citation/sidalc.phtml', $sidalc);
        }

        // If we got this far, it's the default article case:
        [$sidalc['volume'], $sidalc['issue'], $sidalc['date']]
            = $this->getAPANumbersAndDate();
        $sidalc['journal'] = $this->details['journal'];
        $sidalc['pageRange'] = $this->getPageRange();
        return $partial('Citation/sidalc-article.phtml', $sidalc);
    }
    /**
